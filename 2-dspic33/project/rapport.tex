...
The algorithm need 8 034 537 cycles to run. We used the debugger to get that value, which is more precise than a approximation using the sum of individual operation done in the lab.

The time for a cycle depends of the frequency of the internal clock.

If we suppose that the frequency of the microcontroller is 120MHZ as suggested by the debugger, a cycle would take $ \frac{1}{120 * 10^{6} }$.

So the maximum time between two buffer update is $ \frac{1}{120 * 10^{6} } * 8 034 537 = 133.9 ms $

And the maximum frequency for buffer updtae is about 7.46 Hz. 
